...
export const AppReducer = (state, action) => {
    ...
    switch (action.type) {
        case 'ADD_EXPENSE':
            const addedExpense = action.payload;
            const newExpenses = state.expenses.map((currentExp) => {
                if (currentExp.name === addedExpense.name) {
                    return {
                        ...currentExp,
                        cost: currentExp.cost + addedExpense.cost,
                    };
                }
                return currentExp;
            });

            const newTotal = newExpenses.reduce((total, item) => total + item.cost, 0);
